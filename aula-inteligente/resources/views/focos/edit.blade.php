...
<div class="container my-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h1 class="fw-bold" style="color: #5D7B6F;">Editar Foco</h1>
    </div>
        ...
        <form action="{{ route('focos.update', $foco->id) }}" method="POST">
            ...
            <div class="mb-3">
                ...
                <select class="form-select" id="estado" name="estado" required>
                    <option value="1" @if($foco->estado) selected @endif>Encendido</option>
                    <option value="0" @if(!$foco->estado) selected @endif>Apagado</option>
                </select>
            </div>
            
            <div class="mb-3">
                <label for="intensidad" class="form-label" style="color: #5D7B6F;">Intensidad: <span id="intensidadValue">{{ $foco->intensidad }}</span>%</label>
                <input type="range" class="form-range" id="intensidad" name="intensidad" min="0" max="100" value="{{ old('intensidad', $foco->intensidad) }}" oninput="document.getElementById('intensidadValue').innerText = this.value" required>
            </div>
